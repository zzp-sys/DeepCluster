...
from utils.train_utils import junyun_train
...
def main():
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        assigned_dataset, batch_size=256, shuffle=(sampler is None),
        num_workers=8, pin_memory=True, sampler=sampler, drop_last=False)
               
        # Trains
        print('Train ...')

        result_loss, std_feature = junyun_train(assigned_loader, model, criterion, criterion1, criterion_supcon, optimizer, epoch)
        std_data.append([epoch ,std_feature.cpu().numpy()])
        #np.savetxt('',std_feature.cpu().numpy(), fmt='%.6f', delimiter=',')
        ...
        data_write_csv("/root/DeepClustermyproject/std_data/cluster_data.csv", cluster_data)
        data_write_csv("/root/DeepClustermyproject/std_data/std.csv", std_data) #测试
    writer.close()
    #data_write_csv("/root/DeepClustermyproject/std_data/std.csv", std_data)
